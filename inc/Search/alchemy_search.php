...
if(isset($_POST['recipe_name'])){
  ...
  $pages_query = "SELECT * FROM alchemy_recipes_table WHERE recipe_name LIKE '%{$search_value}%'";
  ...
  $sql_query = "SELECT * FROM alchemy_recipes_table WHERE recipe_name LIKE '%{$search_value}%' LIMIT $firstLimit, $recipesPerPage";
    $result = mysqli_query($conn, $sql_query);
    while ($row = mysqli_fetch_assoc($result)) {
      
      
      echo '<tr><td class="img_row" style="width: 10%"><img src="' .$row['recipe_image']. '" height="30" ></td>';
      echo "<td><a class='name_row' href='alchemyDetails.php?id={$row['recipe_id']}'>" .$row['recipe_name']. '</td>';
